  ...
  return (
    <header className="safe-top sticky top-0 z-50 border-b border-paper/10 bg-ink/95 backdrop-blur supports-[backdrop-filter]:bg-ink/90">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:gap-4 sm:px-6 lg:px-8">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <a
            ...
            className="tap-target flex items-center gap-2 border-2 border-brand-green bg-brand-green px-3 py-2 text-xs font-semibold uppercase tracking-wide text-ink transition-colors hover:border-paper hover:bg-paper hover:text-ink sm:px-4 sm:text-sm"
          >
      ...
      {menuOpen && (
        ...
        >
          <ul className="safe-x mx-auto max-w-6xl px-4 py-2">
            {navItems.map((item) => (
              <li key={item.href}>
                <a
                  href={item.href}
